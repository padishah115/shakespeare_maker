...
def get_chars(lines: list[str], training_window) -> list[str]:
    ...
    for line in lines[:training_window]:
        splitline = list(
            line.split(" ")
        )  # introduce special '£' character for linebreak
        words = ["+" + word for word in splitline if word != ""] + [
            "£"
        ]  # introduce '+' character for space
        for word in words:
...
def main(path: str | os.PathLike = "./training-data/shakespeare.txt"):
    ...
    for n in ngram_sizes:
        train_ngram(fpath=path, context=n)
    ...
    for f in features:
        for c in context:
            train_embedding(
                fpath=path, model_type="EMBEDDING", chars=chars, features=f, context=c
            )
